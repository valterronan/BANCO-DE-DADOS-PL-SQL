...
FROM TCONTRATO
GROUP BY Trunc(DATA)
ORDER BY DATA DESC--Ordem data

UPDATE TCONTRATO SET--Atualizando desconto p/ null cod contrato = 4
  DESCONTO = NULL
WHERE COD_CONTRATO = 4;

UPDATE TCONTRATO SET--Atualizando desconto p/ null cod contrato = 3
  DESCONTO = NULL
WHERE COD_CONTRATO = 3;
--Retorna quantidade de registros tabela tcontrato
SELECT Count(*) AS QTDE_REGISTROS
FROM TCONTRATO;
...
--Total do valor e qunatidade de contrato por estado
SELECT ALU.Estado,
       SUM(CON.Total) TOTAL, Count(*) qtde
FROM   TALUNO ALU, TCONTRATO CON
WHERE  ALU.COD_ALUNO = CON.COD_ALUNO
...
--Retorna data e o valor do contrato opor ordem de total desc
