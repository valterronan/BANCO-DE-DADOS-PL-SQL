...
--Concat une o cod_aluno e o nome em uma coluna só neste exemplo
SELECT Concat(COD_ALUNO,NOME) FROM TALUNO;

--nesse caso mostra cod aluno - nome aluno
SELECT COD_ALUNO||' - '||NOME FROM TALUNO;

--Primeira letra maiuscula o resto minuscula
SELECT nome, InitCap(NOME) FROM TALUNO;

--Posiçao da letra r no registro nome na tabela taluno
SELECT nome, InStr(NOME,'R') FROM TALUNO;

--mostra a quantidade de letras(caracteres) do registro nome na tabela taluno
SELECT nome, Length(NOME) FROM TALUNO;

--muda para minuscula todas os registros nome na tabela taluno
SELECT nome, Lower(NOME) FROM TALUNO

--muda para maiuscula todas os registros nome na tabela taluno
